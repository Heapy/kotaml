...
class YamlContentPolymorphicSerializerTest :
    FlatFunSpec({
            ...
            context("parsing polymorphic values with PolymorphismStyle.None") {
                ...
                context("given some input missing without the serializer") {
                    ...
                    test("throws an exception with the correct location information") {
                        val exception =
                            shouldThrow<IncorrectTypeException> {
...
object TestSealedStructureBasedOnContentSerializer : YamlContentPolymorphicSerializer<TestSealedStructure>(
    ...
    override fun selectDeserializer(node: YamlNode): DeserializationStrategy<TestSealedStructure> =
        when (node) {
            is YamlScalar -> {
                TestSealedStructure.InlineSealedString.serializer()
            }

            is YamlMap -> {
                when (val value: YamlNode? = node["value"]) {
                    is YamlScalar -> {
                        when {
                    ...
                    is YamlNull -> {
                        TestSealedStructure.SimpleSealedString.serializer()
                    }

                    else -> {
                        throw SerializationException(
                            "Unsupported property type for TestSealedStructure.value: ${value?.let { it::class.simpleName}}",
                        )
                    }
                }
            }
                ...
                throw SerializationException("Unsupported node type for TestSealedStructure: ${node::class.simpleName}")
            }
        }
}
