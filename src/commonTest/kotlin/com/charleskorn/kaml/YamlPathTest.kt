        ...
        context("a YAML path") {
            context("creating a path") {
                ...
                context("given a list of segments where the root element appears but not as the first element") {
                    test("throws an exception") {
                        val exception =
                            shouldThrow<IllegalArgumentException> { YamlPath(YamlPathSegment.Error(Location(1, 2)), YamlPathSegment.Root) }
                        ...
                        val exception =
                            shouldThrow<IllegalArgumentException> {
                                YamlPath(
                                    ...
                                    YamlPathSegment.Error(Location(1, 2)),
                                    YamlPathSegment.Root,
                                )
                            }
            ...
            context("converting a path to a string suitable for display to a user") {
                ...
                context(
                    "given a path for a reference to the key of a merged object from an alias that was part of a list of objects to merge",
                ) {
                    // eg. << : [ &blue, &green ]
                    val path =
                        YamlPath.root
