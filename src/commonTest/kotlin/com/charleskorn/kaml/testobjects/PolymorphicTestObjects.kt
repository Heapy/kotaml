...
sealed interface TestSealedStructure {
    ...
    data class SimpleSealedInt(
        val value: Int,
    ) : TestSealedStructure

    @Serializable
    @SerialName("sealedString")
    data class SimpleSealedString(
        val value: String?,
    ) : TestSealedStructure
    ...
    @JvmInline
    value class InlineSealedString(
        val value: String,
    ) : TestSealedStructure
}

@Serializable
data class SealedWrapper(
    val element: TestSealedStructure?,
)
...
data class UnsealedBoolean(
    val value: Boolean,
) : UnsealedClass()

@Serializable
@SerialName("unsealedString")
data class UnsealedString(
    val value: String,
) : UnsealedClass()
...
object UnwrappedNull : UnwrappedInterface {
    val kSerializer: KSerializer<UnwrappedNull> =
        UnwrappedValueSerializer(Unit.serializer().nullable, "simpleNull", { UnwrappedNull }, { null })
}

data class UnwrappedUnit(
    val data: Unit,
) : UnwrappedInterface {
    companion object {
...
data class UnwrappedNullableInt(
    val data: Int?,
) : UnwrappedInterface {
    companion object {
        val kSerializer =
            UnwrappedValueSerializer(Int.serializer().nullable, "simpleNullableInt", ::UnwrappedNullableInt, UnwrappedNullableInt::data)
    }
}

data class UnwrappedLong(
    val data: Long,
) : UnwrappedInterface {
    companion object {
...
class UnwrappedValueSerializer<S, T>(
    private val valueSerializer: KSerializer<S>,
    descriptorName: String,
    private val fromSource: (S) -> T,
    private val toSource: (T) -> S,
) : KSerializer<T> {
    override val descriptor: SerialDescriptor = SerialDescriptor(descriptorName, valueSerializer.descriptor)

    override fun deserialize(decoder: Decoder): T = fromSource(valueSerializer.deserialize(decoder))
...
@Serializable
data class PolymorphicWrapper(
    val test: UnwrappedInterface,
)
