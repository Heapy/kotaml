        ...
        context("a YAML parser") {
            ...
            context("parsing nested map node") {
                ...
                context("parsing that input as a map node") {
                    ...
                    test("deserializes map") {
                        ...
                        result.node
                            .get<YamlMap>("foo5")!!
                            .get<YamlScalar>("element1")!!
                            .toInt() shouldBe 1
                        result.node
                            .get<YamlMap>("foo5")!!
                            .get<YamlScalar>("element2")!!
                            .toInt() shouldBe 2
                    }
                }
            ...
            context("parsing values with a dynamically installed serializer") {
                ...
                context("parsing a map with a contextual serializer") {
                    val contextSerializer =
                        object : KSerializer<Inner> {
                            override val descriptor =
                                buildSerialDescriptor("Inner", StructureKind.MAP) {
            ...
            context("parsing values with a contextual serializer") {
                ...
                context("decoding from a YamlNode") {
                    ...
                    val mapAsListSerializer =
                        object : KSerializer<List<Database>> {
                            override val descriptor =
                                buildSerialDescriptor("DatabaseList", StructureKind.MAP) {
                                }
                ...
                context("decoding with a custom serializer for a non-root node") {
                    ...
                    test("decodes the Yaml as an ObjectContainingObjectWithCustomSerializer") {
                        result shouldBe
                            ObjectContainingObjectWithCustomSerializer(
...
private data class CustomSerializedValue(
    val thing: String,
)
...
object ContextualSerializer : KSerializer<String> {
    override val descriptor =
        buildSerialDescriptor("ContextualSerializer", SerialKind.CONTEXTUAL) {
...
class DelegatedContextualSerializer(
    ...
    override fun deserialize(decoder: Decoder): Any = serializer.deserialize(decoder)
...
@OptIn(InternalSerializationApi::class, ExperimentalSerializationApi::class)
class ContextualSerializerThatAttemptsToDeserializeIncorrectType(
...
@Serializable
private data class NullableNestedObject(
    val firstPerson: SimpleStructure?,
)

@Serializable
data class NullableNestedList(
...
private data class DatabaseListing(
    val databases: List<Database>,
)

@Serializable
private data class ServerConfig(
    val databaseListing: DatabaseListing,
)
...
private data class Container(
    @Contextual val inner: Inner,
)

@Serializable
private data class ObjectWithNestedContextualSerializer(
    @Serializable(with = ContextualSerializer::class) val thing: String,
)
...
private object SerializerForObjectWithCustomSerializer : KSerializer<ObjectWithCustomSerializer> {
    ...
    override fun serialize(
        encoder: Encoder,
        value: ObjectWithCustomSerializer,
    ) {
        encoder.encodeSerializableValue(listSerializer, value.combinedValues.split(VALUES_SEPARATOR))
    }
