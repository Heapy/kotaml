...
public sealed class YamlInput(
    ...
    internal companion object {
        private val missingFieldExceptionMessage: Regex =
            """^Field '(.*)' is required for type with serial name '.*', but it was missing$""".toRegex()
        ...
        internal fun createFor(
            ...
            return when (node) {
                ...
                is YamlScalar -> {
                    when {
                        descriptor.kind is PrimitiveKind || descriptor.kind is SerialKind.ENUM -> {
                            YamlScalarInput(
                                node,
                                yaml,
                                context,
                ...
                is YamlList -> {
                    when (descriptor.kind) {
                        is StructureKind.LIST -> {
                            YamlListInput(node, yaml, context, configuration)
                        }
                ...
                is YamlMap -> {
                    when (descriptor.kind) {
                        is StructureKind.CLASS, StructureKind.OBJECT -> {
                            YamlObjectInput(node, yaml, context, configuration)
                        }

                        is StructureKind.MAP -> {
                            YamlMapInput(node, yaml, context, configuration)
                        }
                ...
                is YamlTaggedNode -> {
                    when {
                        ...
                        descriptor.kind is PolymorphicKind && configuration.polymorphismStyle == PolymorphismStyle.Tag -> {
                            ...
                            createFor(node.innerNode, yaml, context, configuration, descriptor)
                        }
                    }
                }
            }
        ...
        private fun createContextual(
            ...
            context
                .getContextualDescriptor(descriptor)
        ...
        private fun createPolymorphicMapDeserializer(
            ...
            when (val typeName = node.getValue(desiredKey)) {
                is YamlList -> {
                    throw InvalidPropertyValueException(desiredKey, "expected a string, but got a list", typeName.path)
                }

                is YamlMap -> {
                    throw InvalidPropertyValueException(desiredKey, "expected a string, but got a map", typeName.path)
                }

                is YamlNull -> {
                    throw InvalidPropertyValueException(desiredKey, "expected a string, but got a null value", typeName.path)
                }
