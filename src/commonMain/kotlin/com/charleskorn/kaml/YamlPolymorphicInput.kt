...
internal class YamlPolymorphicInput(
    val typeName: String,
    private val typeNamePath: YamlPath,
    val contentNode: YamlNode,
    yaml: Yaml,
    ...
    override fun decodeElementIndex(descriptor: SerialDescriptor): Int =
        when (currentField) {
            ...
            CurrentField.Type -> {
                when (contentNode) {
                    is YamlScalar -> {
                        contentDecoder = YamlScalarInput(contentNode, yaml, serializersModule, configuration)
                    }

                    is YamlNull -> {
                        contentDecoder = YamlNullInput(contentNode, yaml, serializersModule, configuration)
                    }
    ...
    override fun beginStructure(descriptor: SerialDescriptor): CompositeDecoder {
        return when (currentField) {
            CurrentField.NotStarted, CurrentField.Type -> {
    ...
    private inline fun <T> maybeCallOnContent(
        functionName: String,
        blockOnContent: YamlInput.() -> T,
    ): T =
        maybeCallOnContent(
    ...
    private fun getKnownTypesForOpenType(className: String): Set<String> {
        ...
        serializersModule.dumpTo(
            object : SerializersModuleCollector {
                ...
                // FIXME: ideally we'd be able to get the name as used by the SerialModule
                // (eg. the values in 'polyBase2NamedSerializers' in SerialModuleImpl,
                // but these aren't exposed.
                ...
                ): Unit = throw UnsupportedOperationException("This method should never be called.")
